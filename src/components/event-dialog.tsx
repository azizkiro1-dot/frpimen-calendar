...
import { Trash2, AlertTriangle, Sparkles, MapPin, FileText, Repeat, Loader2, X } from 'lucide-react'
...
function prettyRange(startDate: string, startTime: string, endDate: string, endTime: string): string {
  ...
  if (sameDay) {
    return `${s.toFormat('EEE, LLL d')} · ${s.toFormat('h:mm a')} – ${e.toFormat('h:mm a')}`
  }
...
export function EventDialog({ open, onOpenChange, meetingTypes, event, defaultDate }: Props) {
  ...
  // When start date/time changes, auto-adjust end to maintain duration (if user hasn't manually touched end)
  useEffect(() => {
    if (isEdit) return
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg p-0 gap-0 max-h-[92vh] overflow-y-auto [&>button]:hidden">
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          <div className="flex items-center gap-2.5">
            {currentType && <span className="h-2.5 w-2.5 rounded-full" style={{ background: currentType.color }} />}
            <DialogTitle className="text-[15px] font-semibold">{isEdit ? 'Edit event' : 'New event'}</DialogTitle>
          </div>
          <button onClick={() => onOpenChange(false)} className="h-8 w-8 rounded-lg flex items-center justify-center hover:bg-neutral-100">
        ...
        <div className="px-5 pb-5 space-y-5">
          <input
            ...
            className="w-full text-lg font-medium placeholder:text-neutral-400 border-0 border-b border-neutral-200 pb-2 focus:outline-none focus:border-neutral-900 bg-transparent"
          />

          <div className="space-y-3">
            <div className="text-[11px] uppercase tracking-wider text-neutral-500 font-medium">Starts</div>
            ...
            <div className="text-[11px] uppercase tracking-wider text-neutral-500 font-medium">Ends</div>
            <div className="grid grid-cols-[1fr_120px] gap-2">
              <Input type="date" value={endDate} onChange={e => onEndDateChange(e.target.value)} className="h-10" />
            ...
            {rangeText && <p className="text-xs text-neutral-500">{rangeText}</p>}
          </div>

          {checkingConflicts && <p className="text-xs text-neutral-500 flex items-center gap-2"><Loader2 className="h-3 w-3 animate-spin" /> Checking conflicts...</p>}
          ...
          {conflicts.length > 0 && (
            <div className="rounded-xl border border-amber-300 bg-amber-50 p-3.5 space-y-3">
              ...
              <ul className="text-[13px] text-amber-900 space-y-1 pl-6 list-disc">
                {conflicts.map(c => {
          ...
          <div className="space-y-2">
            <div className="text-[11px] uppercase tracking-wider text-neutral-500 font-medium flex items-center gap-1.5"><MapPin className="h-3 w-3" /> Location</div>
            <Input value={location} onChange={e => setLocation(e.target.value)} placeholder="Office, Zoom, church..." className="h-10" />
          </div>

          <div className="space-y-2">
            <div className="text-[11px] uppercase tracking-wider text-neutral-500 font-medium flex items-center gap-1.5"><FileText className="h-3 w-3" /> Notes</div>
            <Textarea rows={3} value={description} onChange={e => setDescription(e.target.value)} placeholder="Optional details" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <div className="text-[11px] uppercase tracking-wider text-neutral-500 font-medium">Type</div>
            ...
            <div className="space-y-2">
              <div className="text-[11px] uppercase tracking-wider text-neutral-500 font-medium">Status</div>
          ...
          <div className="space-y-2">
            <div className="text-[11px] uppercase tracking-wider text-neutral-500 font-medium flex items-center gap-1.5"><Repeat className="h-3 w-3" /> Repeats</div>
            <Select value={recurrence} onValueChange={setRecurrence}>
              ...
              <SelectContent>
                ...
                <SelectItem value="monthly_weekday">Monthly on same weekday</SelectItem>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-neutral-100 bg-neutral-50">
          {isEdit ? (
            <Button type="button" variant="ghost" onClick={handleDelete} disabled={pending} className="text-red-600 hover:text-red-700 hover:bg-red-50 h-9">
              <Trash2 className="h-4 w-4 mr-1.5" /> Delete
            </Button>
          ) : <div />}
          <div className="flex gap-2">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={pending} className="h-9">Cancel</Button>
            <Button type="button" onClick={handleSave} disabled={pending} className="h-9">
